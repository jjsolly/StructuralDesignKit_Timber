...
namespace StructuralDesignKitLibrary.EC5
{
    public static class EC5_MemberCheck
    {
        ...
        public static double RectangularMemberCheck(CrossSectionRectangular crossSection, double kmod, double gamma_M, double leff_y, double leff_z, double Fx, double Fy, double Fz, double Mx, double My, double Mz, out List<string> report) 
        {
            ...
            IMaterialTimber crossSectionMaterial = crossSection.Material as IMaterialTimber;
        ...
        public static double RectangularMemberCheck_Fire(CrossSectionRectangular crossSection, double kmod, double gamma_M, double leff_y, double leff_z, int fireMinutes, List<bool> sidesExposed, double Fx, double Fy, double Fz, double Mx, double My, double Mz, out List<string> report)
        {
            //Initialize variables
            report = new List<string>();
            List<double> utilisations = new List<double>();
            ...
            //handle fire modifications
            if(sidesExposed.Count != 4) throw new ArgumentException("The sidesExposed list must contain exactly 4 boolean values");
            CrossSectionRectangular fireCrossSection = crossSection.ComputeReducedCrossSection(fireMinutes, sidesExposed[0], sidesExposed[1], sidesExposed[2], sidesExposed[3]);
            crossSection = fireCrossSection; //override cross-section with the reduced one

            //Add some lines to the report
            report.Add("Rectangular Section Check under Fire Conditions to EN 1995-1");
            ...
            if (sig_t0d > 0.0)
            {
                double check612 = EC5_CrossSectionCheck.TensionParallelToGrain(sig_t0d, crossSectionMaterial, kmod, gamma_M, kht, kl_lvl, true);
            ...
            if (sig_c0d > 0.0)
            {
                ...
                double check614 = EC5.EC5_CrossSectionCheck.CompressionParallelToGrain(sig_c0d, crossSectionMaterial, kmod, gamma_M, true);
            ...
            if (My != 0.0 || Mz != 0.0)
            {
                ...
                double check616 = EC5.EC5_CrossSectionCheck.Bending(sig_myd, sig_mzd, crossSection, crossSectionMaterial, kmod, gamma_M, khy, khz, true);
            ...
            if (Fy != 0.0 || Fz != 0.0)
            {
                ...
                double check617 = EC5.EC5_CrossSectionCheck.Shear(tau_yd, tau_zd, crossSectionMaterial, kmod, gamma_M, true);
            ...
            if (Mx != 0.0)
            {
                ...
                double check618 = EC5.EC5_CrossSectionCheck.Torsion(tau_tord, tau_yd, tau_zd, crossSection, crossSectionMaterial, kmod, gamma_M, true);
            ...
            if (sig_t0d > 0.0 && (sig_myd > 0 | sig_mzd > 0))
            {
                double check623 = EC5.EC5_CrossSectionCheck.BendingAndTension(sig_myd, sig_mzd, sig_t0d, crossSection, crossSectionMaterial, kmod, gamma_M, khy, khz, kht, kl_lvl, true);
            ...
            if (sig_c0d > 0.0 && (sig_myd > 0 | sig_mzd > 0))
            {
                double check624 = EC5.EC5_CrossSectionCheck.BendingAndCompression(sig_myd, sig_mzd, sig_c0d, crossSection, crossSectionMaterial, kmod, gamma_M, khy, khz, true);
            ...
            if (sig_c0d > 0.0)
            {
                double check632 = EC5.EC5_CrossSectionCheck.BendingAndBuckling(sig_myd, sig_mzd, sig_c0d, leff_y, leff_z, crossSection, crossSectionMaterial, kmod, gamma_M, khy, khz, true);
            ...
            {
                double leff_ltb = Math.Max(leff_y, leff_z);
                double check633 = EC5.EC5_CrossSectionCheck.LateralTorsionalBuckling(sig_myd, sig_mzd, sig_c0d, leff_y, leff_z, leff_ltb, crossSection, crossSectionMaterial, kmod, gamma_M, khy, khz, true);
                report.Add("Section check LTB - clause 6.3.3");
                report.Add("Utilisation=" + Math.Round(check633, 2));
                utilisations.Add(check633);
            }
