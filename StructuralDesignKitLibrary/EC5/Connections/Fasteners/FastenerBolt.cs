...
namespace StructuralDesignKitLibrary.Connections.Fasteners
{
	...
	public class FastenerBolt : IFastener
	{
		...
		private double DefineA4cMin(double? angle)
		{
			return 3 * Diameter;
		}
		...
		{
			List<string> CoveredTimber = new List<string>() { "Softwood", "Hardwood", "Glulam", "LVL", "Baubuche", "OSB" };
			if (CoveredTimber.Contains(timber.Type.ToString()))
			{
				if (timber.Type == TimberType.Softwood || timber.Type == TimberType.Hardwood || timber.Type == TimberType.Glulam || timber.Type == TimberType.LVL || timber.Type == TimberType.Baubuche)
				{
				...
				else if (timber.Type == TimberType.OSB)
				{
					...
					else throw new Exception("Embedment strength calculation with OSB, a thickness must be provided");
		...
		public void ComputeWithdrawalStrength(IShearCapacity ConnectionType)
		{
			...
			if (ConnectionType is SingleInnerSteelPlate)
			{
				...
				FaxRk = Math.Min(ComputeBoltTensileStrength(), ComputeWasherCompressiveStrength(fc90k));
			}
