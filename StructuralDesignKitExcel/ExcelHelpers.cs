...
namespace StructuralDesignKitExcel
{
	public static class ExcelHelpers
	{
		...
		// Helper method to help retrieve the tag of a method, given that the attribute order is not constant
		public static string GetTag(System.Reflection.MethodInfo method)
		{
			string value = "";
			...
			if (namedArguments.Count > 0)
			{
				...
				foreach (var argument in method.CustomAttributes.ToList()[0].NamedArguments.ToList())
				{
					...
					{
						value = argument.TypedValue.Value.ToString();
						break;
					}
				}
			}
		...
		// Helper method to help retrieve the description of a method, given that the attribute order is not constant
		public static string GetDescription(System.Reflection.MethodInfo method)
		{
			string value = "";

			var namedArguments = method.CustomAttributes.ToList()[0].NamedArguments.Where(p => p.MemberName == "Description").ToList();
			if (namedArguments.Count > 0)
			{
				var namedArgument = namedArguments[0];
				foreach (var argument in method.CustomAttributes.ToList()[0].NamedArguments.ToList())
				{
					if (argument.MemberName == "Description")
					{
						value = argument.TypedValue.Value.ToString();
						break;
					}
